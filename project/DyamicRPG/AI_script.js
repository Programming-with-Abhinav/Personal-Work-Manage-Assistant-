...
const gameConfig = {
    ...
    healCost: 15,
    blockMpRegen: 10,
    critChance: 0.20, // 20% Critical hit probability
    critMultiplier: 1.5
};
...
let hero = {
    ...
    attack: 16,
    attackAccuracy: 0.90, // 90% accuracy
...
function executePlayerAction(choice) {
    ...
    switch(choice) {
        case "attack":
            if (Math.random() <= hero.attackAccuracy) {
                ...
                if (isCrit) {
                    ...
                    appendLog(`✨ CRITICAL HIT! Hero slashes Boss for ${damage} DMG!`, "hero-msg");
                } else {
        ...
        case "magic":
            ...
            appendLog(`Hero casts Arcane Fire! Spell hits instantly for ${hero.magicDamage} DMG!`, "hero-msg");
            break;

        case "heal":
            hero.mp -= gameConfig.healCost;
            hero.hp = Math.min(gameConfig.maxHeroHp, hero.hp + hero.healValue);
            appendLog(`Hero uses restoration magic. Recovers ${hero.healValue} HP!`, "hero-msg");
            break;

        case "block":
            hero.shieldActive = true;
            hero.mp = Math.min(gameConfig.maxHeroMp, hero.mp + gameConfig.blockMpRegen);
            DOM.heroShieldBadge.classList.remove("hidden");
            appendLog(`Hero focuses stance! Restored +10 MP and raised shield.`, "hero-msg");
            break;
    }
    return true;
}
...
function executeBossAction() {
    ...
    if (boss.action === "heavy") {
        ...
        accuracy = 0.60; // Heavy attack is strong but misses more often (60%)
    } else {
...
function checkGameStatus() {
    if (boss.hp <= 0 && hero.hp <= 0) {
        ...
        appendLog(`Victory! The Boss has been defeated in ${turnCount} turns!`, "hero-msg");
        endGame(true);
    } else if (hero.hp <= 0) {
...
function handleScoreSavings(score) {
    ...
    if (!currentRecord || score < parseInt(currentRecord)) {
        ...
        appendLog("✨ NEW HIGH SCORE! Most efficient run recorded! ✨", "system-msg");
    }
}
